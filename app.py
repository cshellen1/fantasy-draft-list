...
from models import db, connect_db, User, List, PlayerList, Player, UserRegistrationError
...
def update_player_data():
    ...
    r = requests.get(
        f"https://nba-stats-db.herokuapp.com/api/playerdata/season/{curr_date.year}"
    )
    resp = r.json()
...
def show_player_search_details():
    ...
    if request.args.get("q"):
        ...
        players = Player.query.filter(
            unaccent(Player.name).ilike(f"%{player_search}%")
        ).all()
...
def compare_players():
    ...
    if form2.validate_on_submit():
        ...
        pg = (
            ...
            .filter(Player.name.ilike(f"{form2.point_guard.data}"))
            .first()
        )
        sg = (
            db.session.query(Player.id)
            .filter(Player.name.ilike(f"{form2.strong_guard.data}"))
            .first()
        )
        sf = (
            db.session.query(Player.id)
            .filter(Player.name.ilike(f"{form2.small_forward.data}"))
            .first()
        )
        pf = (
            db.session.query(Player.id)
            .filter(Player.name.ilike(f"{form2.power_forward.data}"))
            .first()
        )
        c = (
            db.session.query(Player.id)
            .filter(Player.name.ilike(f"{form2.center.data}"))
            .first()
        )
